...
const printStyles = `
  @media print {
    ...
    .pay-section { border: 1px solid black !important; margin-bottom: 12px !important; }
    .print-page-break { page-break-after: always; }
    .print-page-break-before { page-break-before: always; }
    .print-header { margin-bottom: 12px !important; text-align: center; }
    .print-header h1 { font-size: 18px !important; margin-bottom: 4px !important; font-weight: bold; }
    .print-header h2 { font-size: 16px !important; margin: 4px 0 !important; font-weight: bold; }
    .print-header p { font-size: 11px !important; margin: 2px 0 !important; }
    .print-header .border-t { border-top: 2px solid black !important; border-bottom: 2px solid black !important; padding: 8px 0 !important; margin: 8px 0 !important; }
    .employee-details { margin-bottom: 12px !important; font-size: 11px !important; }
    .employee-details .grid { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 16px !important; }
    .employee-details .space-y-1 > * + * { margin-top: 2px !important; }
    .employee-details p { display: flex !important; margin: 2px 0 !important; }
    .employee-details span:first-child { width: 120px !important; font-weight: 600; flex-shrink: 0; }
    .pay-section { border: 2px solid black !important; margin-bottom: 12px !important; }
    .pay-section h3 { font-size: 12px !important; padding: 6px 8px !important; margin: 0 !important; background: #f0f0f0 !important; border-bottom: 1px solid black !important; font-weight: bold; }
    .pay-section .p-3 { padding: 8px !important; }
    .pay-section .space-y-2 > * + * { margin-top: 3px !important; }
    .pay-section .text-sm { font-size: 11px !important; }
    .pay-section .border-t { border-top: 1px solid black !important; padding-top: 6px !important; margin-top: 6px !important; }
    .pay-details-grid { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 16px !important; }
    .net-pay-section { margin-top: 12px !important; padding: 8px !important; border: 3px solid black !important; background: white !important; }
    .summary-cards .bg-blue-50,
    .summary-cards .bg-green-50,
    .summary-cards .bg-purple-50 { padding: 8px !important; }
    .monthly-section { margin-bottom: 32px !important; }
    .annual-section { margin-top: 32px !important; }
    .print-page-break-before { page-break-before: auto !important; }
    .net-pay-section .text-xl { font-size: 16px !important; font-weight: bold; }
    .net-pay-section .text-sm { font-size: 10px !important; margin-top: 6px !important; }
    .annual-table { font-size: 10px !important; width: 100% !important; border-collapse: collapse !important; margin-top: 12px !important; }
    .annual-table th,
    .annual-table td { border: 1px solid black !important; padding: 4px 6px !important; text-align: left; }
    .annual-table th { background: #e0e0e0 !important; font-weight: bold !important; }
    .annual-table td:not(:first-child) { text-align: right !important; }
    .monthly-grid { display: grid !important; grid-template-columns: repeat(4, 1fr) !important; gap: 8px !important; margin-top: 12px !important; page-break-inside: avoid; }
    .monthly-grid > div { border: 1px solid black !important; padding: 6px !important; font-size: 9px !important; text-align: center; }
    .monthly-grid .selected-month { border: 2px solid black !important; }
    .monthly-grid .font-semibold { font-size: 9px !important; font-weight: bold !important; margin-bottom: 2px !important; }
    .monthly-grid .text-lg { font-size: 11px !important; font-weight: bold !important; }
    .print-only { display: block !important; }
    .screen-only { display: none !important; }
    .print-section { page-break-inside: avoid; margin-bottom: 24px !important; }
    .monthly-section,
    .annual-section { display: block !important; }
    .section-heading { font-size: 14px !important; font-weight: bold !important; margin: 12px 0 8px 0 !important; border-bottom: 1px solid black !important; padding-bottom: 4px !important; }
  }
  @media screen {
...
const earningCategories = [
    "Basic", "House Rent Allowance", "Other Allowance", "Car Allowance",
    "Interest Subsidy", "Performance Pay", "Long Term Incentive Plan",
    "Miscellaneous Allowance", "Mobile Handset Allowance"
];

const defaultMonthlyEarnings = () => Object.fromEntries(earningCategories.map(cat => [cat, 0]));

const initialEmployeeState = {
    name: "", code: "", department: "", designation: "", grade: "", dob: "",
    doj: "", location: "", bank: "", costCenter: "", pan: "", pf: "", esi: "",
...
const Payslip = ({ user }) => {
    ...
    const [deductions, setDeductions] = useState({
        incomeTax: 0, pf: 0, professionalTax: 200, benevolence: 2000,
        esop: 5208, esic: 0, loanRepayment: 0,
    });
    const [deductionToggles, setDeductionToggles] = useState({
        incomeTax: true, pf: true, professionalTax: true, benevolence: true,
...
const handleDeleteEmployee = async (empId) => {
    ...
    useEffect(() => {
        ...
        const monthData = employeeData[selectedEmpId]?.[selectedYear]?.[selectedMonth] || {};
...
const saveCurrentMonthData = async () => {
    ...
    const EmployeeForm = ({ employee, onSave, onCancel }) => {
        ...
        const formFields = [
            { name: 'name', label: 'Name *', placeholder: 'Enter employee name' },
            { name: 'code', label: 'Employee Code *', placeholder: 'Enter employee code' },
            ...
            { name: 'grade', label: 'Grade', placeholder: 'Enter grade' },
            { name: 'dob', label: 'Date of Birth', placeholder: 'e.g., 2 Aug 1979' },
            { name: 'doj', label: 'Date of Joining', placeholder: 'e.g., 18 Jun 2008' },
            { name: 'location', label: 'Location', placeholder: 'Enter location' },
            { name: 'bank', label: 'Bank Details', placeholder: 'Account number (Bank name)' },
            { name: 'costCenter', label: 'Cost Center', placeholder: 'Enter cost center' },
            { name: 'pan', label: 'PAN Number', placeholder: 'Enter PAN number' },
            { name: 'pf', label: 'PF Number', placeholder: 'Enter PF number' },
            { name: 'esi', label: 'ESI Number', placeholder: "Enter ESI number (or '-')" },
            { name: 'pfUan', label: 'PF UAN', placeholder: 'Enter PF UAN' },
            { name: 'heading', label: 'Company Name', placeholder: 'Enter company name' },
            { name: 'address', label: 'Company Address', placeholder: 'Enter company address' },
        ...
        return (
            <Card className={`p-4 mb-6 ${isEditing ? 'bg-blue-50 border-blue-200' : 'bg-gray-50'}`}>
                ...
                <div className="flex gap-2">
                    <Button onClick={() => onSave(formData)} className={isEditing ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 print:hidden">
                    <div>
                        ...
                        <Select value={selectedEmpId} onValueChange={setSelectedEmpId}>
                            ...
                            <SelectContent>{employees.map((emp) => <SelectItem key={emp.id} value={emp.id}>{emp.name} ({emp.code})</SelectItem>)}</SelectContent>
                        </Select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div className="mt-2">
                            <Input type="text" placeholder="Enter year (e.g. 2023 or 2023-24)" value={typedValue}
                                onChange={(e) => {
                    ...
                    <div className="flex items-end"><Button onClick={saveCurrentMonthData} className="w-full bg-green-600 hover:bg-green-700">Save Data</Button></div>
                </div>
